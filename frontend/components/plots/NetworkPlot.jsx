        ...
        if (data) {
            ...
            function displayNodeInfo(node) {
                ...
                infoBox.append("text")
                    .attr("x", 10)
                    .attr("y", 30)
                    .text(`Cluster: ${node.id} Connections`);
                    ...
                    if (outgoingLinks.length > 0) {
                        ...
                        const outgoingNodes = outgoingLinks.map(link => link.target.id).join(", ");
                        infoBox.append("text")
                            .attr("x", 10)
                            .attr("y", yOffset)
                            .text(`-> ${outgoingNodes}`);
                    ...
                    if (incomingLinks.length > 0) {
                        ...
                        const incomingNodes = incomingLinks.map(link => link.source.id).join(", ");
                        infoBox.append("text")
                            .attr("x", 10)
                            .attr("y", yOffset)
                            .text(`<- ${incomingNodes}`);
