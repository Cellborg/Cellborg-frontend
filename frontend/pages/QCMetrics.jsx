...
const QCMetrics = ({data: session, token, datasetName,datasetId, completed}) => {
  ...
  const plots = jsonData? [
    <div>
      <DynamicViolinPlot key={0} plotData={jsonData} datamap={'pct_counts_mt'} className='w-auto'/>
      <DynamicViolinPlot key={0} plotData={jsonData} datamap={'total_counts'} className='w-auto'/>
      <DynamicViolinPlot key={0} plotData={jsonData} datamap={'n_genes'} className='w-auto'/>
    </div>,
